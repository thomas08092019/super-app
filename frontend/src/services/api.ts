...
export const telegramAPI = {
  ...
  getMessages: async (params: {
    ...
    const response = await api.get<Message[]>('/telegram/messages', { params });
    return response.data;
  },
  getGroupsHistory: async (sessionId: number) => {
    const response = await api.get<{id: string, name: string}[]>('/telegram/groups', {
      params: { session_id: sessionId }
    });
    return response.data;
  }
