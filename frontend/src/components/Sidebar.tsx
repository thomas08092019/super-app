...
import { LayoutDashboard, Users, MessageSquare, Bot, Database, Search, Radio, ChevronDown, ChevronRight, LogOut, Menu, Folder, Archive, BookOpen, Brain, Zap } from 'lucide-react';
import { useAuthStore } from '../store/authStore';

interface MenuItem { name: string; path?: string; icon: any; children?: MenuItem[]; adminOnly?: boolean; }

const menuItems: MenuItem[] = [
  { name: 'System', icon: LayoutDashboard, children: [
...
export default function Sidebar() {
  ...
  // Thêm 'Academy' vào trạng thái mở mặc định nếu muốn
  const [expanded, setExpanded] = useState<string[]>(['System', 'Telegram', 'Academy']);
    ...
    <>
      ...
      <motion.aside initial={false} animate={{ x: isOpen ? 0 : -300 }} className="fixed left-0 top-0 h-screen w-64 bg-gray-900 border-r border-gray-800 flex flex-col z-40">
        ...
        <nav className="flex-1 overflow-y-auto p-4 space-y-2 custom-scrollbar">
          {menuItems.map(item => (
            <div key={item.name}>
              <button onClick={() => toggle(item.name)} className="w-full flex items-center justify-between px-4 py-3 rounded-lg hover:bg-gray-800 transition-colors"><div className="flex items-center gap-3"><item.icon size={20} className="text-gray-400" /><span className="font-medium text-gray-200">{item.name}</span></div>{expanded.includes(item.name) ? <ChevronDown size={18} className="text-gray-400" /> : <ChevronRight size={18} className="text-gray-400" />}</button>
              <AnimatePresence>
                  ...
                  <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="overflow-hidden">
                    <div className="ml-4 mt-1 space-y-1">
                      {item.children.map(child => {
                        ...
                        return <Link key={child.path} to={child.path || '#'} className={`flex items-center gap-3 px-4 py-2 rounded-lg transition-colors ${isActive(child.path) ? 'bg-blue-600 text-white' : 'hover:bg-gray-800 text-gray-400 hover:text-white'}`}><child.icon size={18} /><span className="text-sm">{child.name}</span></Link>;
                      })}
                    </div>
                  </motion.div>
                )}
