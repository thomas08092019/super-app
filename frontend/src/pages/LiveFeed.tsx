...
export default function LiveFeed() {
    ...
    useEffect(() => {
        ...
        // Bắt sự kiện thay đổi Group hoặc Session để gọi lại API nếu cần (thay cho logic handleSearch ở dưới)
    ...
    useEffect(() => {
        // Chỉ chạy khi sessionId thay đổi
        setMessages([]);
        setPage(1);
        setHasMore(true);
        loadGroups();
        
        // Chỉ fetch messages nếu không phải lần load đầu tiên (để tránh double call)
    ...
    useLayoutEffect(() => {
        if (scrollRef.current) {
            if (prevScrollHeight > 0) {
                ...
                // Scroll xuống cuối khi tải lần đầu tiên (page 1)
    ...
    const loadSessions = async () => { 
        try { 
            ...
            setSessions(data); 
            // FIXED: Set default session to the first one, or 0 if empty
            if (data.length > 0) setSessionId(data[0].id);
        } catch (e) { console.error(e); } 
    };
    const loadGroups = async () => { 
        try { 
            const data = await telegramAPI.getGroupsHistory(sessionId === 0 ? undefined : sessionId); 
            setGroups(data); 
        } catch (e) { console.error(e); } 
    };

    const fetchMessages = async (pageNum: number, reset: boolean = false, overrideParams: { chat_id?: string, session_id?: number } = {}) => {
        setLoading(true);
        try {
            // FIXED: Ưu tiên tham số override được truyền vào
            const chatFilter = overrideParams.chat_id !== undefined ? overrideParams.chat_id : selectedGroup;
            const targetSession = overrideParams.session_id !== undefined ? overrideParams.session_id : sessionId;
            const targetSessionAPI = targetSession === 0 ? undefined : targetSession;
            const toUTC = (dateStr: string) => dateStr ? new Date(dateStr).toISOString() : undefined;

            const data = await telegramAPI.getMessages({
                session_id: targetSessionAPI, page: pageNum, limit: 20,
    ...
    const connectAllWebSockets = () => {
        ...
        // FIXED: Không kết nối WebSocket nếu SessionId = 0 (All Accounts) để tránh quá tải
        if (sessionId === 0) {
            setConnected(false);
            return;
        }
        ...
        ws.current.onmessage = (event) => {
            ...
            if (data.type === 'message') {
                ...
                // Kiểm tra lại bằng Ref để đảm bảo state không bị lỗi closure
                ...
                // FIXED: Chỉ thêm tin nhắn nếu bộ lọc đang ở "All Groups" hoặc match group đang chọn
    ...
    // FIXED: Tự động điền session ID và gọi API
    const handleFilterByGroup = (chatId: string, msgSessionId: number) => { 
        // 1. Cập nhật SessionID
        setSessionId(msgSessionId); 
        // 2. Cập nhật GroupID
        setSelectedGroup(chatId); 
        // 3. Gọi API (handleSearch sẽ được gọi thông qua useEffect [selectedGroup, sessionId])
    };
    
    const clearGroupFilter = () => { setSelectedGroup(''); };
    
    // ... (các hàm xử lý profile & group khác giữ nguyên)
                        ...
                        <motion.div key={`${msg.id}-${idx}`} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="flex gap-4 group hover:bg-gray-800/30 p-2 rounded-lg transition-colors">
                            ...
                            <div className="flex-1 min-w-0">
                                <div className="flex items-baseline gap-2 mb-1 min-w-0">
                                    ...
                                    {/* FIXED: Truyền msg.session_id vào handleFilterByGroup */}
