...
import { Download, Terminal, Square } from 'lucide-react';
...
export default function Downloader() {
  ...
  const handleStartDownload = async () => {
    if (!sessionId || !chatId) { alert('Please select a session and enter a chat ID'); return; }
    ...
    try {
      ...
      addLog(`Task ID: ${result.task_id} started.`);
    } catch (error: any) { 
      addLog(`Error starting task: ${error.message}`); 
      setIsDownloading(false); 
    }
  };
        ...
        <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.1 }} className="bg-gray-800/50 backdrop-blur-lg rounded-xl p-6 border border-gray-700 h-fit">
          ...
          <div className="space-y-4">
            <div>
              ...
              <select value={sessionId || ''} onChange={(e) => setSessionId(Number(e.target.value))} disabled={isDownloading} className="w-full px-4 py-2 bg-gray-900 border border-gray-600 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 disabled:opacity-50">
            ...
            <div>
              ...
              <ChatSelector sessionId={sessionId} value={chatId} onChange={setChatId} placeholder="Enter chat ID or select..." />
            </div>
            <div>
              <label className="block text-sm font-medium text-gray-300 mb-2">Limit (Optional)</label>
              <input type="number" value={limit} onChange={(e) => setLimit(e.target.value)} placeholder="Leave empty for all files" disabled={isDownloading} className="w-full px-4 py-2 bg-gray-900 border border-gray-600 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 disabled:opacity-50" />
              ...
              <div className="grid grid-cols-2 gap-2">
                {ALL_MEDIA_TYPES.map(type => (
                  <label key={type} className="flex items-center gap-2 cursor-pointer hover:text-white transition-colors">
                    <input type="checkbox" checked={mediaTypes.includes(type)} onChange={() => toggleMediaType(type)} disabled={isDownloading} className="w-4 h-4 rounded" />
            ...
            {isDownloading ? (
                ...
                <button onClick={handleStartDownload} disabled={!sessionId || !chatId} className="w-full flex items-center justify-center gap-2 px-4 py-3 bg-blue-600 hover:bg-blue-700 rounded-lg font-medium transition-all disabled:opacity-50 disabled:cursor-not-allowed">
