...
async def start_dump(
    ...
    dump_task = DumpTask(
        ...
        task_id="pending", 
        status="pending",
        target_date=request.start_time or datetime.utcnow()
    )
    db.add(dump_task)
    await db.commit()
    ...
    task = dump_messages_task.apply_async(
        args=[request.session_id, request.chat_id, request.start_time, request.end_time, dump_task.id]
    )
